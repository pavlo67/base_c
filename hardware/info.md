...
`gpio/gpio.h` declares the abstract `Gpio` class. The `hardware_gpio` CMake target selects the implementation for the platform. `gpio/_defines.h` contains the programmer-selected BCM pin numbers for automatic tests and the GPIO probe, together with PWM probe settings.
