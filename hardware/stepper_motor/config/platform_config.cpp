...
constexpr const char* ON_LOAD_STEPPER_PROBE_CONFIG = "[loadStepperMotorProbeConfig()]";

bool loadStepperMotorProbeConfig(const Config& config, StepperMotorProbeConfig& probe) {
    StepperMotorProbeConfig candidate;
    if (!loadPlatformMotorConfig(config, candidate.motors_)) { return false; }
    try {
        const auto kindNode = config.get("motorClass");
        const std::string kind = kindNode ? kindNode.as<std::string>() : "Smart";
        if (kind == "Smart") {
            candidate.kind_ = StepperMotorKind::smart;
        } else if (kind == "Dumb") {
            candidate.kind_ = StepperMotorKind::dumb;
        } else {
            printf("%s ERROR: motorClass must be Smart or Dumb\n", ON_LOAD_STEPPER_PROBE_CONFIG);
            return false;
        }
        if (candidate.kind_ == StepperMotorKind::dumb) {
            for (size_t i = 0; i < candidate.dumb_.size(); ++i) {
                const char* axis = i == 0 ? "pan" : "tilt";
                const auto node = config.get(axis);
                auto& dumb = candidate.dumb_[i];
                dumb.speedDegPerSec_ = node["dumbSpeedDegSec"].as<float>();
                const auto pauseMs = node["dumbPauseMs"].as<uint64_t>();
                dumb.remainingPulsesTolerance_ = node["dumbRemainingPulsesTolerance"].as<uint64_t>();
                if (!std::isfinite(dumb.speedDegPerSec_) || dumb.speedDegPerSec_ <= 0 ||
                        pauseMs >= candidate.motors_[i].timeLimit_ / MILLISECOND) {
                    printf("%s ERROR: %s: invalid Dumb speed or pause\n", ON_LOAD_STEPPER_PROBE_CONFIG, axis);
                    return false;
                }
                dumb.pauseAfterSeries_ = pauseMs * MILLISECOND;
            }
        }
    } catch (const YAML::Exception& error) {
        printf("%s ERROR: invalid probe configuration: %s\n", ON_LOAD_STEPPER_PROBE_CONFIG, error.what());
        return false;
    }
    probe = candidate;
    return true;
