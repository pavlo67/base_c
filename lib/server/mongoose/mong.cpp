...
bool sameMethod(SERVER_HTTP_METHOD method, const mg_str& actualMethod) {
    const char* expected = nullptr;

    switch (method) {
        case SERVER_HTTP_METHOD::GET:     expected = "GET"; break;
        case SERVER_HTTP_METHOD::POST:    expected = "POST"; break;
        case SERVER_HTTP_METHOD::PUT:     expected = "PUT"; break;
        case SERVER_HTTP_METHOD::DELETE:  expected = "DELETE"; break;
        case SERVER_HTTP_METHOD::PATCH:   expected = "PATCH"; break;
        case SERVER_HTTP_METHOD::OPTIONS: expected = "OPTIONS"; break;
        case SERVER_HTTP_METHOD::HEAD:    expected = "HEAD"; break;
    }
...
void startServer(uint32_t ipV4Host, uint16_t port) {
    ...
    fprintf(stderr, "%sstarted on %s:%u\n", ON_START_SERVER.c_str(), host.c_str(), static_cast<unsigned>(port));
}
