...
const std::string ON_JLIST_WRITE = "[jlistWrite()]";
...
bool jlistWrite(const std::string& filepath, const Json::Value& jv, bool addNewLine) {
    ...
    if (fptr == nullptr) {
        printf("%s ERROR: can't open %s for appending", ON_JLIST_WRITE.c_str(), filepath.c_str());
        return false;
    }
...
const std::string ON_JLIST_WRITE_ALL = "[jlistWriteAll()]";
...
bool jlistWriteAll(const std::string& filepath, const Json::Value& jvHeader, const Json::Value& jvList) {
    ...
    if (fptr == nullptr) {
        printf("%s ERROR: can't open %s for writing", ON_JLIST_WRITE_ALL.c_str(), filepath.c_str());
        return false;
    }
...
const std::string ON_JLIST_READ_ALL = "[jlistReadAll()]";
bool jlistReadAll(const std::string& filepath, Json::Value& jvHeader, Json::Value& jvList, bool ignoreErrors) {
    ...
    for (int i = 0; i < lines.size(); i++) {
        ...
        if (pos == J_HEADER_KEY.length() && line.substr(0, pos) == J_HEADER_KEY) {
            if (headerOk) {
                printf("%s ERROR: header line duplicate is omitted / %s", ON_JLIST_READ_ALL.c_str(), line.c_str());
            } else if (reader.parse(line.substr(pos + J_KEY_DELIMITER.length()), jvHeader)) {
                headerOk = true;
            } else {
                printf("%s ERROR: header line is wrong / %s", ON_JLIST_READ_ALL.c_str(), line.c_str());
                if (!ignoreErrors) {
                    return false;
                }
            }
            ...
            if (reader.parse(line, jv)) {
                ...
                printf("%s ERROR: can't parse jlist line #%d (of %lu total lines): '%s'\n\n", ON_JLIST_READ_ALL.c_str(), i, lines.size(), line.c_str());
                if (!ignoreErrors) {
                    return false;
                }
...
const std::string ON_JSON_READ = "[jsonRead()]";
bool jsonRead(const std::string& filepath, Json::Value& jv) {
    ...
    printf("%s ERROR: can't read json (%s) from %s\n", ON_JSON_READ.c_str(), json.c_str(), filepath.c_str());
    return false;
}

const std::string ON_JSON_WRITE = "[jsonWrite()]";
